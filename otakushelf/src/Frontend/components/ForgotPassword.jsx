...
const ForgotPassword = () => {
    ...
    const handleResetSubmit = async (e) => {
        e.preventDefault();
        if (!otp) { setResetMessage("Please enter the verification code."); setResetStatus("error"); return; }
        ...
        try {
            const res = await axios.post(`${API}/auth/reset-password`, { email: forgotEmail, otp, newPassword });
            setResetMessage(res.data.message);
            setResetStatus("success");
            setStep("success");
            setTimeout(() => navigate("/login"), 3000);
        } catch (err) {
            setResetMessage(err.response?.data?.message || "Invalid or expired code. Please request a new one.");
    ...
    return (
        <div className="login-container">
            ...
            >
                ...
                >
                    <div className="login-card-icon">{step === "success" ? "✅" : step === "otp" ? "🛡️" : "🔑"}</div>
                ...
                >
                    ...
                    {step === "otp" && (
                        <>
                            <h1 className="login-auth-title">Reset Password</h1>
                            <p style={{ color: "rgba(255,255,255,0.55)", fontSize: "14px", margin: "8px 0 0" }}>
                                Enter the code sent to <strong style={{ color: "#a78bfa" }}>{forgotEmail}</strong>
                            </p>
                        </>
                    )}
                    {step === "success" && (
                        <h1 className="login-auth-title">Password Reset</h1>
                    )}
                </motion.div>
                        ...
                        >
                            ...
                            >
                                {forgotStatus === "loading" ? (
                                    ...
                                    <span className="login-btn-text">Send Verification Code</span>
                                )}
                            </motion.button>
                        </motion.form>
                    )}
